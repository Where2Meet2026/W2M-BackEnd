...
├─ meeting/           모임 생성/조회/삭제, 초대 코드 발급, 약속 시간 확정 (2학기: 최종 확정 장소 정보도 여기 추가 예정 — "최종확정"은 별도 도메인이 아니라 vote의 마지막 투표 처리 결과를 meeting이 들고 있는 구조)
├─ participant/       모임 참여자 정보 (방장/참여자 역할), 참여·탈퇴·목록 조회
├─ availability/      참여자별 "가능한 시간대" 저장/조회
├─ recommendation/    참여자들의 가능 시간을 모아 공통 시간대 계산 및 추천
│
├─ location/          (2학기) 참여자별 출발 위치 좌표 저장 (시간 확정 후 1회만 입력, 날짜별 저장 아님)
├─ candidate/         (2학기) 중간지점 주변 실제 장소 후보 산출 (빠른/균형/최적) + 후보 좋아요·싫어요 반응
├─ vote/              (2학기) 후보 최종 투표 집계, 전원 투표 완료 시 자동 확정 로직도 여기 포함
├─ review/            (2학기) 확정된 장소에 대한 익명 후기(별점+한줄평) — 누적되면 candidate의 "최적" 점수 계산에 반영
